...
| `apps/web/vitest.config.ts` | Added `execArgv: ["--no-experimental-webstorage"]` to the `test` block (one line) | Node 22+ exposes a native, non-functional `globalThis.localStorage` accessor that vitest's jsdom environment refuses to overwrite (it only copies a jsdom `window` key onto `global` when the key is absent or allow-listed), so every `window.localStorage.clear()` in the suite hit Node's stub and threw. The flag disables Node's Web Storage global in the vitest worker so jsdom's Storage is used; a config seam, no test file touched. Shop-ops ledger 2026-08-23 entry. Keep this line across upstream merges of `vitest.config.ts`. |
...
**Same-day review follow-up on `apps/web/nginx.kaneo.conf`:** the `map` gained a third branch, `~\.(svg|png|ico|webmanifest|txt|xml)$ "no-cache"`, so the un-hashed files Vite copies from `apps/web/public/` (favicon, manifest, logos, mascot, email lockups) revalidate instead of falling back to heuristic freshness; `/assets/` stays first because nginx tries map regexes in order of appearance. Re-verified in `nginx:alpine`: `/assets/app-abc123.js` and `/assets/img-abc.png` immutable, `/`, `/index.html`, `/dashboard/foo`, `/favicon.svg`, `/site.webmanifest` `no-cache`, `/api/x` no Cache-Control, all four security headers present.
...
`pnpm --filter @kaneo/api test` — `Test Files  60 passed (60)`, `Tests  406 passed (406)` (after the same-branch follow-up commit that corrected `tests/api/utils/get-invitation-email-subject.test.ts`: its fr-FR/de-DE/pt-BR cases still asserted `"... on Kaneo"` while `i18n/{fr-FR,de-DE,pt-BR}.json` had read `"... on Anota"` since Phase 07 Plan 08 — a stale test, not a product regression).
`pnpm --filter @kaneo/web test` — `Test Files  47 passed (47)`, `Tests  189 passed (189)` (after the cherry-picked `vitest.config.ts` fix on this branch: Node 22+ exposes a native, non-functional `globalThis.localStorage` that vitest's jsdom environment refuses to overwrite, so `src/hooks/use-board-sort.test.tsx` and `use-task-filters-with-labels-support.test.tsx` threw `Cannot read properties of undefined (reading 'clear')`; `test.execArgv: ["--no-experimental-webstorage"]` unshadows jsdom's Storage — shop-ops ledger 2026-08-23 entry).
